...
from SkipNoMaxWrapper import SkipNoMax
...
class Args:
    ...
    wandb_project_name: str = "fixed-code"
...
6 - on the fly update (1 max-q-range frame prompt)
7 - 6 + no kickstart loss to check for correctness
same project
...
def find_imp_states(q_table, q_network, device, struggling_states):
    ...
    struggling_frame = str(struggling_frame.tolist())
    return (struggling_frame,si,spi)
...
Question:I'm playing a game of Atari Pong-v4 using the openai gym environment of grid size 210x210 with origin at the bottom-left corner.""")
    ...
    impvalues = torch.zeros((args.num_steps, args.num_envs)).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        #     if rec_action==-1:
        #         failed_states+=1
        #         save_img(torch.tensor(ast.literal_eval(ss)).unsqueeze(0).to(device),failed_states)
        #         continue
        ...
        for step in range(0, args.num_steps):
            ...
            if infos and "final_info" not in infos:
                prev_info_arr[step] = info_arr[step-1]
            else:
                prev_info_arr[step] = None
            ...
            #Generate important values
            #loop through obs tensors, use q-network to calculate important values and append to important values list 
            q_values = q_network.model(next_obs)
            impvalue = torch.max(q_values)-torch.min(q_values).to(device)
            impvalues[step] = impvalue
            ...
            if global_step % 1000 == 0:
                ...
                writer.add_scalar("charts/test_return", y[-1], x[-1])
        ...
        #**********************important values**************************
        b_impvalues = impvalues.reshape(-1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                ###########################################################################################
                #calculate the ranking of the important values for all the samples in the mini_batch for top 10 important values and querry LLM 
                #b_impvalues[b_inds] sort the list to get the top 10 important values 
                mini_b_impvalues = b_impvalues[mb_inds]
                imp_inds = torch.topk(mini_b_impvalues, 1).indices
                #generate_teacher_logits(top 10 important values states) -> 10 tensors with prob of actions
                #querry llm here
                #We don't need struggling_states anymore                
                teacher_probs = np.zeros((args.minibatch_size, 6))
                kickstarting_loss=0
                # for i in imp_inds:
                #     s_info = info_arr[i]
                #     sp_info = prev_info_arr[i]
                #     rec_action = teacher.prompt([(s_info, sp_info)])
                #     if rec_action==-1:
                #         continue
                #     teacher_probs[i][rec_action] = 1
                # teacher_probs = torch.from_numpy(np.array(teacher_probs)).to(device)
                # kickstarting_loss = -(logits * teacher_probs).sum(dim=-1).mean()
                # if kickstarting_loss.item()!=0:
                ...
                #Do we apply the kl to all states or just the important values states 
        ...
        # writer.add_scalar("losses/kickstarting_loss", kickstarting_loss.item(), global_step)
