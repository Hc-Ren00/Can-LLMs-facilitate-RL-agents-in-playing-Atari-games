    ...
    def ppo_update(self, ppo_epochs, mini_batch_size, states, actions, log_probs, returns, advantages, clip_param=0.2):
        for _ in range(ppo_epochs):
            for state, action, old_log_probs, return_, advantage in self.ppo_iter(mini_batch_size, states, actions, log_probs, returns, advantages):
                ...
                kickstarting_loss = 0#-(dist.logits * self.generate_teacher_logits(state)).sum(dim=-1).mean()
    ...
    def train(self, frame_idx):
        ...
        while frame_idx < self.max_frames and not self.early_stop:
            ...
            for _ in range(self.num_steps):
                ...
                self.curr_state, _ = preprocess_image(self.curr_state)#torch.FloatTensor(self.curr_state).to(self.device)
                ...
                next_state, reward, terminated, truncated, info  = self.env.step(action.cpu().numpy()[0])
            ...
            next_state, _ = preprocess_image(next_state)
            ...
            for state1,value,action,adv in zip(states,values,actions,advantage):
                ...
                if str_tensor not in self.local_q_buffer:
                    self.local_q_buffer[str_tensor]={0:0,1:0,2:0,3:0}
                q_value = adv.tolist()[0]+value.tolist()[0]
                self.local_q_buffer[str_tensor][action.tolist()]=adv.tolist()[0]+value.tolist()[0]
